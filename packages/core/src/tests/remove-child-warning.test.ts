...
// wait is paid once.
test("only a child that was left with no parent at all is warned about", async () => {
  ...
  createRoot(function Root() {
    ...
    orphaned = oldParent.createChild(Plain);
  });
  ...
  try {
    ...
    destroyed.destroy();
    orphaned.parent!.removeChild(orphaned);
  ...
  // Removed and then left with no parent, which is the leak the warning is for.
  expect(about(orphaned).length).toBe(1);
  expect(String(about(orphaned)[0][0])).toContain(
    "wasn't destroyed within 1 second"
  );

  // Both of the ways the warning's own message suggests resolving it.
  expect(about(reParented).length).toBe(0);
